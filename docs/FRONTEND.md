...
/workspace/calendar           Calendar month view — synced events from Google Calendar connector, task due dates, quick task creation on selected date
/workspace/email-drafts       Email drafts surface — list/compose/edit/send email drafts generated by Larry or created manually, linked to projects and actions
/workspace/larry              Dedicated Larry chat — conversation list, project-scoped and global modes, file attachments, inline action cards with accept/dismiss
/workspace/notifications      Full notification centre — filterable by source (larry, email, slack, calendar), mark-read, search
/workspace/settings           Settings root redirect
/workspace/settings/connectors Connector settings (Slack, Google Calendar, Email)
/workspace/settings/members   Organization member management — list, invite by email, role assignment (admin/member/viewer), remove
/workspace/settings/reliability Runtime reliability + operator recovery (canonical-event retries)
/workspace/settings/larry     Larry autonomy policy — 5-level autonomy slider, confidence thresholds, auto-execute toggle, custom behavioral rules CRUD
/workspace/settings/account   Account settings — display name, avatar upload, password change, active session list with revoke
```
...
| `apps/web/src/app/workspace/calendar/page.tsx` | Calendar month grid with synced events, task due dates, and quick task creation |
| `apps/web/src/app/workspace/email-drafts/EmailDraftsClient.tsx` | Email drafts list, compose/edit form, send flow, linked project/action context |
| `apps/web/src/app/workspace/larry/page.tsx` | Dedicated Larry chat surface with conversation sidebar, project picker, file attachments, inline action cards |
| `apps/web/src/app/workspace/notifications/page.tsx` | Full notification centre with source filter, search, mark-read, relative timestamps |
| `apps/web/src/app/workspace/settings/SettingsSubnav.tsx` | Shared settings sub-navigation (Connectors / Members / Reliability / Larry / Account) |
...
| `apps/web/src/app/workspace/settings/account/page.tsx` | Account settings: profile editing, avatar upload, password change, active session list with device parsing and revoke |
| `apps/web/src/app/workspace/settings/larry/page.tsx` | Larry autonomy policy settings: 5-level slider, confidence thresholds, auto-execute toggle, custom rules CRUD |
| `apps/web/src/app/workspace/settings/members/page.tsx` | Organization member management: list, invite form, role editing (admin/member/viewer), remove with confirmation |
...
| `apps/web/src/hooks/useCalendarEvents.ts` | Fetches calendar events for the month-view calendar page |
...
- `POST /larry/events/:id/let-larry-execute` — let Larry auto-execute a suggested event
- `POST /larry/events/:id/modify` — modify a suggested event before accepting
...
- `GET /larry/documents` — Larry-generated documents with `projectId`, `docType`, `limit` filters
- `GET /larry/documents/:id` — single Larry document detail
...
- `PATCH /documents/:id/move` — move document into a folder
- `POST /documents/generate` — generate template document (`project_status` | `task_export` | `project_brief` in `docx` | `xlsx` | `pptx`)
...
- `PATCH /folders/:id/move` — move folder to new parent
- `GET /tasks/:id` — single task detail with comments
...
- `GET /tasks/:id/comments` — task comment list
- `POST /tasks/:id/comments` — add task comment
- `PATCH /tasks/:id/status` — update task status (`not_started` | `on_track` | `at_risk` | `overdue` | `completed`)

**Other active routes:**
- `GET /home` — workspace home composite (active projects, archived projects, tasks, connector statuses)
...
- `GET /projects/:id/dashboard` — project dashboard composite (health + outcomes + task breakdown + status history)
- `GET /projects/:id/timeline` — project timeline entries
- `GET /projects/:id/outcomes` — project outcomes
- `POST /projects/:id/archive` — archive project
- `POST /projects/:id/unarchive` — unarchive project
- `POST /projects/:id/delete` — delete project
- `POST /projects` — create project
- `GET /meetings` — meeting list with optional `?projectId=` and `?limit=` filters
- `GET /meetings/:id` — single meeting detail
- `GET /meetings/overview` — meetings overview with active project enrichment
...
- `GET /my-work` — cross-project task view for current user (active projects + tasks)
- `GET /activity` — activity feed with optional `?limit=` filter
- `GET /search` — workspace search across projects, tasks, and documents (`?q=` required)
- `GET /notifications` — notifications with `?unread=true` filter
- `POST /notifications/:id/read` — mark notification as read
- `GET /members` — organization member list
- `POST /members/invite` — invite member by email with role (`admin` | `member` | `viewer`)
- `PATCH /members/:userId` — update organization member role
- `DELETE /members/:userId` — remove organization member
- `GET /email/drafts` — list email drafts with optional `?state=` and `?limit=` filters
- `POST /email/drafts/send` — compose and send email draft (linked to project/action)
- `GET/PATCH /settings/policy` — Larry autonomy policy (autonomy level, confidence thresholds, auto-execute toggle)
- `GET/POST /settings/rules` — Larry custom behavioral rules
- `PATCH/DELETE /settings/rules/:id` — update or delete a custom rule
- `GET /connectors/summary` — connector install URLs for Slack, Google Calendar, Outlook Calendar, Email
- `GET /connectors/:connector/install` — initiate connector OAuth install flow
- `GET /connectors/slack/channels` — Slack channel list for mapping
- `GET/PUT /connectors/slack/channel-mapping` — Slack channel-to-project mapping
- `GET/PUT /connectors/calendar/project-link` — Google Calendar-to-project linking
- `POST /actions/:id/correct` — legacy action correction route (still active, proxies to `/v1/larry/actions/:id/correct`)
...
- `POST /actions/:id/approve`, `/reject` — retired; use `/larry/events/:id/accept` or `/dismiss`
- `GET /actions`, `GET /actions/:id` — retired; use `/larry/action-centre` or `/projects/:id/action-centre`
