...
export async function runEscalationScan() {
    try {
        ...
        const tenantRows = await db.tx(async (client) => {
            ...
            const r = await client.query("SELECT id FROM tenants");
            return r.rows;
        });
        for (const tenant of tenantRows) {
            const tenantId = tenant.id;
            const tasks = await db.queryTenant(tenantId, `SELECT id, title, status, due_date, start_date, progress_percent, updated_at, assignee_user_id
         FROM tasks
         WHERE tenant_id = $1
           AND status NOT IN ('completed', 'backlog')`, [tenantId]);
            const notifications = [];
            ...
            const notifiedUserIds = [...new Set(notifications.map((n) => n.userId).filter(Boolean))];
            ...
            if (notifiedUserIds.length > 0) {
                try {
                    const userRows = await db.tx(async (client) => {
                        await client.query("SELECT set_config('app.tenant_id', $1, true)", ["__system__"]);
                        const r = await client.query("SELECT id, email FROM users WHERE id = ANY($1::uuid[])", [notifiedUserIds]);
                        return r.rows;
                    });
            ...
            try {
                const slackRows = await db.queryTenant(tenantId, "SELECT bot_access_token FROM slack_installations WHERE tenant_id = $1 LIMIT 1", [tenantId]);
            ...
            for (const notif of notifications) {
                try {
                    await db.queryTenant(tenantId, `INSERT INTO notifications
             ...
             ON CONFLICT ON CONSTRAINT uq_notifications_dedup DO NOTHING`, [
                ...
                if (slackBotToken && userEmail) {
                    try {
                        ...
                        const lookupRes = await fetch(`https://slack.com/api/users.lookupByEmail?email=${encodeURIComponent(userEmail)}`, { headers: { Authorization: `Bearer ${slackBotToken}` } });
