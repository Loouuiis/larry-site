    ...
    const workers = Array.from({ length: Math.min(SCAN_CONCURRENCY, projectRows.length) }, async () => {
        while (queue.length > 0) {
            ...
            try {
                ...
                console.error(`[larry-scan] project ${projectId} (tenant ${tenantId}) failed`, err instanceof Error ? err.message : String(err));
            }
        }
    });
    await Promise.all(workers);
    console.log(`[larry-scan] Processed ${processed} projects, executed ${totalExecuted} actions, stored ${totalSuggested} suggestions`);
}
