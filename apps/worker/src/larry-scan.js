...
async function recordJobHeartbeat(jobName, startedAt, stats) {
    try {
        const now = new Date();
        await db.query(`INSERT INTO system_job_runs
...
export async function runLarryScan() {
    ...
    const projectRows = await db.tx(async (client) => {
        ...
        const r = await client.query(`SELECT p.id, p.tenant_id
       FROM projects p
    ...
    const workers = Array.from({ length: Math.min(SCAN_CONCURRENCY, projectRows.length) }, async () => {
        while (queue.length > 0) {
            ...
            try {
                ...
                const ledgerContext = { sourceKind: "project_review", sourceRecordId: randomUUID() };
                ...
                console.error(`[larry-scan] project ${projectId} (tenant ${tenantId}) failed`, message);
            }
        }
    });
    await Promise.all(workers);
    const elapsedSeconds = ((Date.now() - startTime) / 1000).toFixed(1);
    console.log(`[larry-scan] completed in ${elapsedSeconds}s — processed: ${processed}, failed: ${failed}, actions: ${totalExecuted}`);
