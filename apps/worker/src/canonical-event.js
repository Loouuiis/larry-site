...
async function loadCanonicalEvent(tenantId, canonicalEventId) {
    const rows = await db.queryTenant(tenantId, `SELECT id, source, payload
     FROM canonical_events
     WHERE tenant_id = $1
       AND id = $2
     LIMIT 1`, [tenantId, canonicalEventId]);
    return rows[0] ?? null;
}
...
async function reconcileMeetingNote(tenantId, meetingNoteId, actionCount, options = {}) {
    await db.queryTenant(tenantId, `UPDATE meeting_notes
     SET project_id = COALESCE(project_id, $3),
         summary = COALESCE($4, summary),
         action_count = $5
     WHERE tenant_id = $1
       AND id = $2`, [tenantId, meetingNoteId, options.projectId ?? null, options.summary ?? null, actionCount]);
}
function buildTranscriptPrompt(transcript) {
...
function readCalendarProjectHint(payload) {
...
function readSlackProjectHint(payload, event) {
...
function buildSlackPrompt(payload, event) {
...
function buildMemorySummary(prefix, briefing) {
...
async function loadSlackMappedProjectId(tenantId, slackTeamId, slackChannelId) {
    const rows = await db.queryTenant(tenantId, `SELECT project_id
     FROM slack_channel_project_mappings
     WHERE tenant_id = $1
       AND slack_team_id = $2
       AND slack_channel_id = $3
     ORDER BY updated_at DESC
     LIMIT 1`, [tenantId, slackTeamId, slackChannelId]);
    return rows[0]?.project_id ?? null;
}
async function loadCalendarMappedProjectId(tenantId, channelId) {
    const rows = await db.queryTenant(tenantId, `SELECT project_id
     FROM google_calendar_installations
     WHERE tenant_id = $1
       AND webhook_channel_id = $2
     ORDER BY updated_at DESC
     LIMIT 1`, [tenantId, channelId]);
    return readOptionalString(rows[0]?.project_id ?? null);
}
async function upsertSlackChannelProjectMapping(tenantId, input) {
    await db.queryTenant(tenantId, `INSERT INTO slack_channel_project_mappings
     ...
     ON CONFLICT (tenant_id, slack_team_id, slack_channel_id) DO UPDATE SET
       ...
       updated_at = NOW()`, [tenantId, input.slackTeamId, input.slackChannelId, input.projectId]);
}
async function handleTranscriptCanonicalEvent(tenantId, canonicalEvent) {
    const transcriptPayload = canonicalEvent.payload;
    ...
    const resolvedProjectId = readOptionalString(transcriptPayload.projectId) ?? meetingNote?.project_id ?? null;
    ...
    if (!resolvedProjectId) {
        console.warn(`[canonical-event] transcript event ${canonicalEvent.id} has no resolvable project scope; skipping action generation`);
        return;
    }
    if (await shouldSkipArchivedProjectWrites({
        tenantId,
        canonicalEvent,
        projectId: resolvedProjectId,
    })) {
...
async function handleEmailCanonicalEvent(tenantId, canonicalEvent) {
    const emailPayload = canonicalEvent.payload;
    const projectId = readOptionalString(emailPayload.projectId);
    if (!projectId) {
        console.warn(`[canonical-event] email event ${canonicalEvent.id} missing projectId; skipping action generation`);
        return;
    }
    if (await shouldSkipArchivedProjectWrites({
        tenantId,
    ...
    try {
        ...
        const reason = error instanceof Error ? error.message : String(error);
        console.warn(`[canonical-event] email event ${canonicalEvent.id} has invalid projectId ${projectId}; skipping (${reason})`);
        return;
    }
    const config = buildWorkerIntelligenceConfig();
    ...
    await Promise.all([
        runAutoActions(db, tenantId, projectId, "signal", intelligenceResult.autoActions, undefined, ledgerContext),
        storeSuggestions(db, tenantId, projectId, "signal", intelligenceResult.suggestedActions, undefined, ledgerContext),
        Promise.resolve(insertProjectMemoryEntry(db, tenantId, projectId, {
            source: "Email signal",
            sourceKind: "email",
            sourceRecordId: canonicalEvent.id,
            ...
            const reason = error instanceof Error ? error.message : String(error);
            console.warn(`[canonical-event] email event ${canonicalEvent.id} failed to write project memory; continuing (${reason})`);
        }),
    ]);
}
async function handleCalendarCanonicalEvent(tenantId, canonicalEvent) {
    const calendarPayload = canonicalEvent.payload;
    ...
    if (!resolvedProjectId) {
        console.warn(`[canonical-event] calendar event ${canonicalEvent.id} has no resolvable project scope; skipping action generation`);
        return;
    }
    if (await shouldSkipArchivedProjectWrites({
        tenantId,
    ...
    try {
        ...
        const reason = error instanceof Error ? error.message : String(error);
        console.warn(`[canonical-event] calendar event ${canonicalEvent.id} has invalid project scope ${resolvedProjectId}; skipping (${reason})`);
        return;
    }
    const config = buildWorkerIntelligenceConfig();
    ...
    await Promise.all([
        ...
        Promise.resolve(insertProjectMemoryEntry(db, tenantId, resolvedProjectId, {
            ...
            const reason = error instanceof Error ? error.message : String(error);
            console.warn(`[canonical-event] calendar event ${canonicalEvent.id} failed to write project memory; continuing (${reason})`);
        }),
    ]);
}
async function handleSlackCanonicalEvent(tenantId, canonicalEvent) {
    const slackPayload = canonicalEvent.payload;
    const event = readOptionalRecord(slackPayload.event);
    const slackTeamId = readOptionalString(slackPayload.team_id) ?? readOptionalString(event?.team);
    ...
    if (!resolvedProjectId) {
        console.warn(`[canonical-event] slack event ${canonicalEvent.id} has no resolvable project scope; skipping action generation`);
        return;
    }
    if (await shouldSkipArchivedProjectWrites({
        tenantId,
        ...
        if (hintedProjectId && slackTeamId && slackChannelId) {
            ...
            if (mappedProjectId && mappedProjectId !== resolvedProjectId) {
                try {
                    ...
                    console.warn(`[canonical-event] slack event ${canonicalEvent.id} has invalid fallback project scope ${mappedProjectId}; skipping (${fallbackReason})`);
                    return;
                }
            }
            else {
                const reason = error instanceof Error ? error.message : String(error);
                console.warn(`[canonical-event] slack event ${canonicalEvent.id} has invalid hinted project scope ${resolvedProjectId}; skipping (${reason})`);
                return;
            }
        }
        else {
            const reason = error instanceof Error ? error.message : String(error);
            console.warn(`[canonical-event] slack event ${canonicalEvent.id} has invalid project scope ${resolvedProjectId}; skipping (${reason})`);
            return;
        }
    }
    if (hintedProjectId && slackTeamId && slackChannelId && resolvedProjectId === hintedProjectId) {
        await upsertSlackChannelProjectMapping(tenantId, {
            ...
            const reason = error instanceof Error ? error.message : String(error);
            console.warn(`[canonical-event] slack event ${canonicalEvent.id} failed to upsert channel mapping; continuing (${reason})`);
        });
    }
    const config = buildWorkerIntelligenceConfig();
    ...
    await Promise.all([
        ...
        Promise.resolve(insertProjectMemoryEntry(db, tenantId, resolvedProjectId, {
            ...
            const reason = error instanceof Error ? error.message : String(error);
            console.warn(`[canonical-event] slack event ${canonicalEvent.id} failed to write project memory; continuing (${reason})`);
        }),
    ]);
}
export async function handleCanonicalEventCreated(tenantId, payload) {
    const job = payload;
