...
export async function runCalendarWebhookRenewal(): Promise<void> {
  ...
  try {
    ...
    const rows = await db.tx(async (client) => {
      ...
      const r = await client.query<CalendarInstallationRow>(
        `SELECT id, tenant_id, google_calendar_id, google_access_token, google_refresh_token,
        ...
        await db.queryTenant(
          ...
          `UPDATE google_calendar_installations
           SET webhook_channel_id = $3,
               ...
               updated_at = NOW()
           WHERE tenant_id = $1 AND id = $2`,
          [row.tenant_id, row.id, watch.channelId, watch.resourceId, expirationIso]
        );

        console.log(
          `[calendar-renewal] renewed tenant=${row.tenant_id} calendar=${row.google_calendar_id} ` +
          `newChannel=${watch.channelId} expiry=${expirationIso ?? "unknown"}`
        );
        renewed++;
      } catch (err) {
        console.error(
          `[calendar-renewal] failed for installation id=${row.id} tenant=${row.tenant_id}`,
          err
        );
        failed++;
      }
