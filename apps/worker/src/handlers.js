import { finalizeCanonicalEventProcessingAttempt, startCanonicalEventProcessingAttempt, } from "@larry/db";
...
function readCanonicalEventId(payload) {
    const value = payload.canonicalEventId;
    return typeof value === "string" && value.trim().length > 0 ? value : null;
}
function readCanonicalEventSource(payload) {
...
async function processCanonicalEventCreatedJob(job) {
    ...
    try {
        ...
        await finalizeCanonicalEventProcessingAttempt(db, job.data.tenantId, {
            ...
            console.warn(`[worker] failed to finalize canonical_event attempt as succeeded (attemptId=${attempt.id}): ${reason}`);
        });
    }
    catch (error) {
        const status = attemptNumber >= maxAttempts ? "dead_lettered" : "retryable_failed";
        await finalizeCanonicalEventProcessingAttempt(db, job.data.tenantId, {
            attemptId: attempt.id,
            status,
            ...
            console.warn(`[worker] failed to finalize canonical_event attempt as ${status} (attemptId=${attempt.id}): ${reason}`);
        });
        throw error;
    }
}
