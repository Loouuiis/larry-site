...
export function DocumentsPage() {
  ...
  return (
    <motion.div variants={container} initial="hidden" animate="visible" className="space-y-5 pb-10">
      ...
      >
        {/* Header */}
        <div className="hidden sm:grid grid-cols-[2fr_1fr_1fr_40px] border-b border-neutral-100 px-5 py-3 text-[10px] font-semibold uppercase tracking-widest text-neutral-400">
          <span>Document</span>
          <span>Project</span>
          <span>Created</span>
          <span />
        ...
        {loading ? (
          <div className="px-5 py-10 text-center text-xs text-neutral-400">Loading…</div>
        ) : filtered.length === 0 ? (
          <div className="px-5 py-10 text-center text-xs text-neutral-400">
          ...
          <ul role="list" className="divide-y divide-neutral-50">
            {filtered.map((doc) => (
              ...
              >
                ...
                <span className="hidden sm:block truncate text-xs text-neutral-500">
                  {doc.projectName ?? "—"}
                </span>
