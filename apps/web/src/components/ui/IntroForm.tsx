...
type FieldKey = "firstName" | "lastName" | "email" | "company" | "jobTitle" | "comment";
...
function validate(form: FormState): FieldErrors {
  ...
  if (form.comment.length > 1000) errors.comment = "Keep it under 1000 characters";
  return errors;
}
...
export function IntroForm() {
  ...
  if (status === "success") {
    return (
      ...
      >
        ...
        <div>
          <p className="text-lg font-semibold text-[var(--text-1)]">Request received.</p>
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="space-y-5">
      ...
      {/* Comment — textarea, not a Field since it needs a different element */}
      <div className="space-y-1.5">
        <label htmlFor={`${uid}-comment`} className="block text-xs font-medium tracking-wide text-[var(--text-muted)] uppercase">
        ...
        <textarea
          ...
          onChange={(e) => {
            ...
            if (errors.comment) setErrors((er) => ({ ...er, comment: undefined }));
          }}
          rows={4}
          maxLength={1000}
          placeholder="Anything we should know before our call?"
          className={[
            "w-full rounded-xl px-4 py-3 text-[var(--text-1)] outline-none resize-none",
            "bg-white/30 backdrop-blur-sm",
            "border border-white/50",
            "placeholder:text-[var(--text-disabled)]",
            "transition-all duration-200",
      ...
      {/* Marketing consent */}
      <label className="flex items-start gap-3 text-sm text-[var(--text-2)] cursor-pointer">
        <input
          type="checkbox"
          checked={form.marketingConsent}
          onChange={(e) => setForm((f) => ({ ...f, marketingConsent: e.target.checked }))}
          className="mt-0.5 h-4 w-4 accent-[#6c44f6] shrink-0"
        />
      ...
      <p className="text-xs text-[var(--text-disabled)]">
        ...
        <a href="#" className="underline text-[var(--text-muted)] hover:text-[var(--text-1)] transition-colors">
          Privacy Policy
        </a>
        .
      </p>
      ...
      <p className="text-center text-[11px] text-[var(--text-disabled)]">
        No spam. We&apos;ll use your details only to follow up on your intro request.
      </p>
    </form>
  );
}
