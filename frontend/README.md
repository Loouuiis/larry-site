...
## Dedicated Backend (New)
A new enterprise backend service is now scaffolded under `../backend/` (Fastify + Postgres + AI workflow foundations).

Quick commands:
- `npm run backend:dev` (from repo root)
- `npm run backend:migrate` (from repo root)
- `npm run backend:test` (from repo root)
